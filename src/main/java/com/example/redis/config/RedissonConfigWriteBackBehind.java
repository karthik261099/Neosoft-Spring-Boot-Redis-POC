...
@Configuration
public class RedissonConfigWriteBackBehind implements InitializingBean {

    private final String CACHE_NAME="users";

    private RedissonClient redissonClient;

    Logger logger = LoggerFactory.getLogger(RedissonConfigWriteBackBehind.class);
    ...
    public RMapCache<Long, User> userRMapCache() {
        final RMapCache<Long, User> userRMapCache = redissonClient.getMapCache(
                CACHE_NAME,MapOptions.<Long, User>defaults()
                .writer(getMapWriter()).loader(getMapLoader())
                .writeMode(MapOptions.WriteMode.WRITE_BEHIND)
                .writeBehindDelay(30000)//in milliseconds
        ...
        return new MapWriter<Long, User>() {
            ...
            public void write(final Map<Long, User> map) {
                ...
                map.forEach( (k, v) -> {
                    ...
                    User userStoredInDB = userJPAAdapter.createUser(v);

                    System.out.println("Key:"+userStoredInDB.getId()+"  Value:"+userStoredInDB);
                    System.out.println("======================");
